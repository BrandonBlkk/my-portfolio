...
const TimelineItem = ({ timeline, startDate, endDate, durationClass, duration, title, description, link }) => {
    return (
        <div className={timeline}>
            ...
            <button className={`${link === '' ? 'hidden' : 'bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-800 py-1 px-3 rounded-md font-semibold mt-1 hover:bg-slate-300 transition-colors duration-150 select-none'}`}  onClick={() => document.getElementById(link).showModal()}>View result</button>
        </div>
    )
}
